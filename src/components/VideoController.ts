  ...
  private setupShortcuts(): void {
    ...
    this.keydownListener = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        // Seek controls
        //
        // The page binds its own ±10s skip to the arrows. The `handled` block
        // below stops the event before it reaches the page, which only works
        // because this listener sits on `window` (see the addEventListener call).
        case 'arrowleft':
